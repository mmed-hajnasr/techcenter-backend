...
class AdminResearchersApiRegressionTest extends ApiRegressionTestSupport {
        ...
        void adminCanListResearchersFilterUpdateAndDelete() throws Exception {
                ...
                mockMvc.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders
                                .get("/admin/researchers")
                                .header("Authorization", bearerToken(adminToken))
                                .param("domainIds", aiDomain.getDomaineId().toString(),
                                                roboticsDomain.getDomaineId().toString()))
                                .andExpect(status().isOk())
        ...
        @Test
        void nonAdminCanListButCannotManageResearchers() throws Exception {
                ...
                String createPayload = """
                                {
                                  "name": "new researcher",
                                  "biographie": "Should fail"
                                }
                                """;

                postJson("/admin/researchers", userToken, createPayload)
