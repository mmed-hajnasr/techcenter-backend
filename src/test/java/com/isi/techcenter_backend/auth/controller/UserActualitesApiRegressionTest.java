...
class UserActualitesApiRegressionTest extends ApiRegressionTestSupport {
        ...
        void authenticatedUserCanListActualites() throws Exception {
                ...
                getWithToken("/user/actualites", userToken)
                                ...
                                .andExpect(jsonPath("$[0].photoUrl").isEmpty())
                                .andExpect(jsonPath("$[?(@.actualiteId == '%s')]".formatted(createdActualiteId))
                                                .isArray());
        }

        @Test
        void listActualitesWithoutTokenReturnsOk() throws Exception {
